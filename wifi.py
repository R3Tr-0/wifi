...
class wifi():
    ...
    def distract_hoes(self, ap, mac):
        print '[*] setting up a fake access point\n'
        if ap == 'rand':
            ap = str(RandMAC())

        dick = Dot11(subtype=8, type=0, addr1='ff:ff:ff:ff:ff:ff', addr2=str(mac), addr3=str(mac))
        fake_dick = Dot11Beacon(cap="ESS")
        fake_id = Dot11Elt(ID='SSID', info=ap, len=len(str(ap)))
        boom = RadioTap() / dick / fake_dick / fake_id
        print '[+] {} is ready'.format(ap)
        sendp(boom, iface=self.iface, loop=1, verbose=False)
